...
def setup_annotations(rgz_root):
    setup_something(rgz_root, 'data/Annotations', 'anno')
...
def sync_annotations(rgz_root):
    ...
    """
    cwd = os.getcwd()
    anno_path = osp.join(rgz_root, 'data/Annotations')
    os.chdir(anno_path)
    #
    index_path = osp.join(rgz_root, 'data/ImageSets/Main')
    missing_files = []
...
def find_demo_images(rgz_root):
    ...
    """
    image_path = osp.join(rgz_root, 'data/JPEGImages')
    index_path = osp.join(rgz_root, 'data/ImageSets/Main')
    first_id_set = set()
    for indf in os.listdir(index_path):
...
def setup_png_images(rgz_root):
    png_rel = 'data/JPEGImages'
    for img_nm in ['d1_img']:
